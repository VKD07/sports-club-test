...
<template>
  <div
    class="bg-primary-500 dark:bg-muted-800 border-muted-200 dark:border-muted-700 fixed start-0 top-0 z-[60] flex h-full flex-col border-r transition-all duration-300"
    :class="[
      ...
      <ul v-if="startMenuItems?.length" class="space-y-2">
        ...
        <li v-for="(item, index) in startMenuItems" :key="index">
          ...
          <NuxtLink
            v-else-if="item.to"
            :to="item.to"
            exact-active-class="!bg-primary-700 dark:!bg-muted-700/60 !text-muted-100  dark:!text-primary-200"
            class="nui-focus dark:dark:text-muted-400/80 hover:bg-primary-600 dark:hover:bg-muted-700/60 dark:hover:text-muted-100 flex cursor-pointer items-center gap-4 rounded-lg py-3 text-white transition-colors duration-300 hover:text-white"
      ...
      <ul v-if="endMenuItems?.length" class="space-y-2">
        ...
        <li v-for="(item, index) in endMenuItems" :key="index">
          ...
          <NuxtLink
            v-else-if="item.to"
            :to="item.to"
            exact-active-class="!bg-primary-700 dark:!bg-muted-700/60 !text-muted-600 dark:!text-primary-200"
            class="nui-focus text-muted-500 dark:text-muted-400/80 hover:bg-primary-600 dark:hover:bg-muted-700/60 dark:hover:text-muted-100 flex cursor-pointer items-center gap-4 rounded-lg py-3 transition-colors duration-300 hover:text-white"
            :class="!isOpen ? 'px-1 justify-center' : 'px-4'"
          >
            <Icon :name="item.icon.name" :class="item.icon.class" />
            <span
