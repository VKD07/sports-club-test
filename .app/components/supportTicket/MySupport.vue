...
const { data, error, pending, refresh } = await useSupportTicketService(
  '/api/support-ticket/my-support-ticket',
  { query },
)
if (error.value) {
...
const handleClickReopen = (ticket) => {
  if (ticket?.is_exceed) {
    toggleExceedModal()
  } else {
    selectTicket(ticket, 'reopen')
  }
}
      ...
      <div class="flex gap-4 lg:items-center justify-between p-4 my-2 flex-col lg:flex-row">
        <div>
          ...
          <BaseText size="sm" class="text-muted-600 dark:text-muted-400">
            Track all your support inquiries in a single table for easy reference.
          </BaseText>
        </div>

        <div class="flex items-center gap-2">
          <BaseButton color="primary" class="w-full sm:w-32" to="/support-ticket/create">
            <span>Create</span>
          </BaseButton>
        </div>
      </div>
              ...
              >
                ...
                <template #end>
                  ...
                  <FlexTableCell label="action" :hide-label="index > 0">
                    ...
                    >
                      <BaseDropdownItem
                        :to="`/support-ticket/${ticket.id}?tab=My Support`"
                        title="Manage"
                        text="Manage ticket"
                        shape="smooth"
                      />
                      ...
                      <BaseDropdownItem
                        ...
                        text="Reopen ticket"
                        shape="smooth"
                        @click="handleClickReopen(ticket)"
                      />
                    </BaseDropdown>
                  </FlexTableCell>
                </template>
