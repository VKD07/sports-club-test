...
const schoolUserRoles = ref<string[]>([])
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    const { error } = await useApiFetch<any>('/api/admin/create-school-admin-account', {
      method: 'POST',
      body: formData,
      headers: {
    ...
    <BaseCard class="!min-h-[600px]">
      <div class="flex items-center justify-between px-8 py-8 pb-4">
        <div>
          ...
          >
            School User Registration
          </BaseHeading>

          <BaseText size="sm" class="text-muted-600 dark:text-muted-400">
            Create a new school staff account by entering essential details.
          </BaseText>
        ...
        >
          ...
          <div class="border-muted-200 dark:border-muted-700 mt-auto border-t pt-3 space-y-2">
            <div class="flex items-end gap-2">
              ...
              />
              <span class="text-muted-200 dark:text-muted-300 text-sm">{{
                values.school ? values.school : 'School'
              }}</span>
            </div>
