...
// Function to format segments data
const formatSegments = (segmentsData: any) => {
  return segmentsData.map((segment: any) => ({
    id: segment.id,
    src: segment.icon,
    name: segment.name,
    parentCategory: segment.parent_segment?.name,
    numberOfTasks: segment.tasks?.[0]?.task_count ?? 0,
    status: segment.is_active ? 'Active' : 'Inactive',
    createdDate: formatDateFromString(segment.created_at),
  }))
}
...
const parentSegments = formatSegments(segments?.value?.segments?.data)
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    const { error } = await useApiFetch<any>('/api/admin/segments', {
      method: 'POST',
      body: JSON.stringify({
        parent_segment_id: values?.primaryCategory?.id,
        ...
        <div class="flex items-center justify-between px-8 py-8 pb-4">
          <div>
            ...
            <BaseText size="sm" class="text-muted-600 dark:text-muted-400">
              Enter the necessary details to create a new challenge segment. Ensure all fields are
                    ...
                    <div v-else>
                      ...
                      >
                        <div class="flex items-center">
                          <div>
                            <img
                              class="size-14 rounded-md object-cover object-center w-24"
