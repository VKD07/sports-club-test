...
definePageMeta({
  title: 'Add Task (Core Skills)',
  path: '/tasks/add-core-skills-old',
  middleware: ['auth'],
})
...
const skillsSchema = z.object({
  ...
  // skill: z
  //   .object({
  //     id: z.string().min(1, 'Skill is required'),
  //     // name: z.string(),
  //   })
...
const consoleSchema = z.object({
  console: z.object({
    name: z.string().min(1, { message: 'Console name is required' }),
    icon: z.string().min(1, { message: 'Console icon is required' }),
...
const zodSchema = z.object({
  ...
  final_task: z.object({
    ...
    upload_evidence: z.string().min(1, 'Upload evidence is required'),
...
const taskCategory = ref(['Core skills', 'Creative sieries'])
...
const getCombinedSkills = (skills: any, values: any) => {
  if (!Array.isArray(skills)) {
    return []
  }

  const combinedSkills = skills.map((skill) => {
    const matchedSkill = values?.skills_learning?.skills.find((s: any) => s.name === skill.name)
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    const gamingConsoles = {
      description: values?.gaming_consoles.description,
      consoles: values?.gaming_consoles.consoles.map((console) => ({
        url: console.url,
        name: console.console.name,
      })),
      image: values?.gaming_consoles.image,
      score: values?.gaming_consoles.score,
    }
    ...
    const body = {
      ...
      description: values?.task_details?.name,
      banner: values?.task_details?.main_banner,
      thumbnail: values?.task_details?.thumbnail,
      difficulty: values?.introduction_game?.difficulty,
      age: values?.introduction_game?.age,
      // segment_id: values?.task_details?.category?.id,
      sections: {
        introduction: values?.introduction_game,
        skills_learning: {
          description: values?.skills_learning?.description,
          image: values?.skills_learning?.image,
          score: values?.skills_learning?.score,
          skills: combinedSkills,
        },
        scoring_direction: values?.scoring_direction,
        gaming_consoles: gamingConsoles,
        core_activity: values?.final_task,
        knowledge_qa: values?.knowledge_qa,
      },
      segments: values?.task_details?.category?.map((category: any) => {
        return {
          segment_id: category?.id,
        }
      }),
      skills: values?.skills_learning?.skills?.map((skill: any) => {
        return {
          skill_id: getIdFromName(skill?.name, skills?.value?.skills?.data),
        }
      }),
      games: [
        {
          game_id: getIdFromName(values?.introduction_game?.game, games?.value?.games?.data),
        },
      ],
      calendars: [],
    }

    const { error } = await useApiFetch<any>('/api/admin/tasks', {
      method: 'POST',
...
const gamingConsoles = ref<{ console: string; url: string }[]>([
  {
    console: '',
    url: '',
  },
...
const addConsoles = () => {
  ...
  if (gamingConsoles.value.length < 5) {
    gamingConsoles.value.push(newConsole)
  }
}

const removeConsoles = (index: number) => {
  if (values.gaming_consoles?.consoles) {
    const updatedConsoles = [...values.gaming_consoles?.consoles]
    updatedConsoles.splice(index, 1)
    gamingConsoles.value = updatedConsoles
    setFieldValue('gaming_consoles.consoles', gamingConsoles.value)
  }
}
...
const addBanner = (field: any, folder) => {
  uploadField.value = field
  fileFolder.value = folder
  isMediaModalOpen.value = true
}

const addThumbnail = (field: any, folder) => {
  uploadField.value = field
  fileFolder.value = folder
  isMediaModalOpen.value = true
}
...
<template>
  <div>
    ...
    <form method="POST" action="" class="w-full pb-16" @submit.prevent="onSubmit">
            ...
            <div class="grid grid-cols-12 gap-6">
              ...
              <div class="col-span-12 sm:col-span-6">
                ...
                <div v-if="!values.task_details?.main_banner">
                  <BaseButton
                    color="primary"
                    @click="addBanner('task_details.main_banner', 'admin_banners/task_banners')"
                  >
                    ...
                    <div class="flex items-center gap-2">
                      <div class="shrink-0">
                        <img
                          ...
                          :src="getImageUrl(values.task_details?.main_banner)"
                          alt="Image preview"
                        />
                      </div>
          ...
          <TairoFormGroup label="Introduction for the game">
            <div class="grid grid-cols-12 gap-6">
              ...
              <div class="col-span-12 sm:col-span-12">
                <Field
                  v-slot="{ field, errorMessage, handleChange, handleBlur }"
                  name="introduction_game.impact"
                >
                  <CustomQuillEditor
                    fieldName="introduction_game.impact"
            ...
            <div class="grid grid-cols-12 gap-6">
              ...
              <div class="col-span-12 sm:col-span-6">
                ...
                <div v-if="!values.skills_learning?.image">
                  <BaseButton
                    color="primary"
                    @click="addBanner('skills_learning.image', 'admin_banners/task_skills')"
                  >
                    <Icon name="icon-park-outline:upload-picture" class="me-1 size-4" />
                    <span>Add Section Image</span>
                  </BaseButton>
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <div class="shrink-0">
                        <img
                          class="size-14 rounded-md object-cover object-center w-16"
                          :src="getImageUrl(values.skills_learning?.image)"
                          alt="Image preview"
                        />
                      </div>
            ...
            <div class="grid grid-cols-12 gap-6">
              ...
              <div class="col-span-12 sm:col-span-6">
                ...
                <div v-if="!values.scoring_direction?.image">
                  <BaseButton
                    color="primary"
                    @click="addBanner('scoring_direction.image', 'admin_banners/task_score')"
                  >
                    <Icon name="icon-park-outline:upload-picture" class="me-1 size-4" />
                    <span>Add Section Image</span>
                  </BaseButton>
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <div class="shrink-0">
                        <img
                          class="size-14 rounded-md object-cover object-center w-16"
                          :src="getImageUrl(values.scoring_direction?.image)"
                          alt="Image preview"
                        />
                      </div>
      ...
      <!-- GAMING CONSOLES AND DOWNLOADS -->
      <BaseCard class="mt-6">
        <div class="max-w-3xl p-8 py-12">
          <TairoFormGroup label="Gaming consoles and downloads">
            <div class="grid grid-cols-12 gap-6">
              <div class="col-span-12 sm:col-span-12">
                <Field
                  v-slot="{ field, errorMessage, handleChange, handleBlur }"
                  name="gaming_consoles.description"
                >
                  <CustomQuillEditor
                    fieldName="gaming_consoles.description"
                    label="Section description"
                    placeholder="Write some description..."
                    :content="field.value"
                    @update:model-value="handleChange"
              ...
              <div class="col-span-12 sm:col-span-12">
                ...
                <div v-for="(game, index) in gamingConsoles" :key="index">
                  <div class="grid grid-cols-12 mb-3 sm:gap-4">
                    <div class="col-span-12 sm:col-span-4">
                      <Field
                        v-slot="{ field, errorMessage, handleChange, handleBlur }"
                        :name="`gaming_consoles.consoles[${index}].console`"
                      >
                        <BaseListbox
                          :items="consoles"
                    ...
                    <div class="col-span-12 sm:col-span-7">
                      <Field
                        v-slot="{ field, errorMessage, handleChange, handleBlur }"
                        :name="`gaming_consoles.consoles[${index}].url`"
                      >
                        <BaseInput
                          :model-value="field.value"
                          :error="errorMessage"
                    ...
                    <div class="col-span-12 sm:col-span-1">
                      <BaseButtonIcon
                        v-if="index === 0 && gamingConsoles.length === 1"
                        color="primary"
                        class="w-full"
                        @click="addConsoles"
                      >
                      ...
                      <BaseButtonIcon
                        v-if="index === gamingConsoles.length - 1 && gamingConsoles.length > 1"
                        color="primary"
                        class="w-full"
                        @click="addConsoles"
                      >
                      ...
                      <BaseButtonIcon
                        v-else-if="gamingConsoles.length > 1"
                        color="danger"
                        class="w-full"
                        @click="removeConsoles(index)"
                      >
                        <Icon name="lucide:minus" class="size-5" />
                      </BaseButtonIcon>
                    </div>
                ...
                <div>
                  <div class="grid grid-cols-12 mb-3 sm:gap-4">
                    <div class="col-span-12 sm:col-span-3 flex items-center">
                      ...
                      >
                        <Icon name="uil:android-alt" class="w-5 h-5" />
                        <span>Android :</span>
                      </BaseText>
                    </div>
                    ...
                    <div class="col-span-12 sm:col-span-9">
                      <Field
                        v-slot="{ field, errorMessage, handleChange, handleBlur }"
                        :name="`gaming_consoles.consoles.url`"
                      >
                        <BaseInput
                          :model-value="field.value"
                          :error="errorMessage"
                  ...
                  <div class="grid grid-cols-12 mb-3 sm:gap-4">
                    <div class="col-span-12 sm:col-span-3 flex items-center">
                      <BaseText
                        ...
                        class="text-muted-600 dark:text-muted-400 flex gap-2 items-center"
                      >
                        <Icon name="mingcute:apple-line" class="w-5 h-5" />
                        <span>Apple :</span>
                      </BaseText>
                    </div>
              ...
              <div class="col-span-12 sm:col-span-6">
                ...
                <div v-if="!values.gaming_consoles?.image">
                  <BaseButton
                    color="primary"
                    @click="addBanner('gaming_consoles.image', 'admin_banners/task_consoles')"
                  >
                    <Icon name="icon-park-outline:upload-picture" class="me-1 size-4" />
                    <span>Add Section Image</span>
                  </BaseButton>
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <div class="shrink-0">
                        <img
                          class="size-14 rounded-md object-cover object-center w-16"
                          :src="getImageUrl(values.gaming_consoles?.image)"
                          alt="Image preview"
                        />
                      </div>
                    ...
                    <div class="flex gap-2">
                      <button
                        ...
                        type="button"
                        tooltip="Remove"
                        @click.prevent="removeBanner('gaming_consoles.image')"
                      >
                        <Icon name="lucide:x" class="size-4" />
                        <span class="sr-only">Remove</span>
                      </button>
          ...
          <TairoFormGroup label="Final task">
            <div class="grid grid-cols-12 gap-6">
              <div class="col-span-12 sm:col-span-12">
                <Field
                  v-slot="{ field, errorMessage, handleChange, handleBlur }"
                  name="final_task.description"
                >
                  <CustomQuillEditor
                    fieldName="final_task.description"
                    label="Section description"
                    placeholder="Write some description..."
                    :content="field.value"
                    @update:model-value="handleChange"
              ...
              <div class="col-span-12 sm:col-span-12">
                <Field
                  v-slot="{ field, errorMessage, handleChange, handleBlur }"
                  name="final_task.instruction"
                >
                  <CustomQuillEditor
                    fieldName="final_task.instruction"
              ...
              <div class="col-span-12 sm:col-span-12">
                <Field
                  v-slot="{ field, errorMessage, handleChange, handleBlur }"
                  name="final_task.upload_evidence"
                >
                  <CustomQuillEditor
                    fieldName="final_task.upload_evidence"
              ...
              <div class="col-span-12 sm:col-span-6">
                ...
                <div v-if="!values.final_task?.image">
                  <BaseButton
                    color="primary"
                    @click="addBanner('final_task.image', 'admin_banners/task_core_activity')"
                  >
                    <Icon name="icon-park-outline:upload-picture" class="me-1 size-4" />
                    <span>Add Section Image</span>
                  </BaseButton>
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <div class="shrink-0">
                        <img
                          class="size-14 rounded-md object-cover object-center w-16"
                          :src="getImageUrl(values.final_task?.image)"
                          alt="Image preview"
                        />
                      </div>
                    ...
                    <div class="flex gap-2">
                      <button
                        ...
                        type="button"
                        tooltip="Remove"
                        @click.prevent="removeBanner('final_task.image')"
                      >
                        <Icon name="lucide:x" class="size-4" />
                        <span class="sr-only">Remove</span>
                      </button>
