...
const fetchUser = async () => {
  const { data, pending, error, refresh } = await useApiFetch(`/api/admin/admin/${id}`)
...
const initialValues = computed(() => ({
  ...
  picture: selectedUser.value?.admin_profile?.profile_picture || '',
  designation: selectedUser.value?.admin_profile?.designation || '',
  department: JSON.parse(selectedUser.value?.department) || [],
  organization: selectedUser.value?.admin_profile?.organization || '',
  role: selectedUser.value?.role || '',
  whatsappNumber: selectedUser.value?.admin_profile?.whatsapp_no || '',
}))
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    const { error } = await useApiFetch('/api/admin/edit-internal-admin-account', {
      method: 'POST',
      body: formData,
      headers: {
...
const avatarSrc = ref(selectedUser.value?.admin_profile?.profile_picture || '')
    ...
    <BaseCard class="!min-h-[600px]">
      <div class="px-8 pt-8 pb-4 relative">
        ...
        >
            ...
            <div class="px-8 pt-8 pb-4">
              <div class="flex gap-16 justify-between">
                <div class="items-center">
                  <div>
                    ...
                    >
                      {{ values.firstName }}
                      {{ values.lastName }} Account Details
                    </BaseHeading>

                    <BaseText size="sm" class="text-muted-600 dark:text-muted-400">
                      Oversee and update admin user information, including personal, professional,
                ...
                <div class="flex gap-2 items-center">
                  ...
                  >
                    <BaseDropdownItem
                      ...
                      :title="
                        createActionMenu(selectedUser?.status, 'Account', 'Active')?.buttonText
                      "
                      color="default"
                      rounded="sm"
                    ...
                    <BaseDropdownItem
                      v-if="selectedUser?.status === 'active'"
                      :title="
                        createActionMenu(selectedUser?.status, 'Account', 'Suspend')?.buttonText
                      "
                      color="default"
                      rounded="sm"
                    ...
                    <BaseDropdownItem
                      ...
                      :title="
                        createActionMenu(selectedUser?.status, 'Account', 'Release')?.buttonText
                      "
                      color="default"
                      rounded="sm"
                    ...
                    <BaseDropdownItem
                      ...
                      :title="
                        createActionMenu(selectedUser?.status, 'Account', 'Resume')?.buttonText
                      "
                      color="default"
                      rounded="sm"
                    ...
                    <BaseDropdownItem
                      v-if="
                        selectedUser?.status === 'draft' || selectedUser?.status === 'suspended'
                      "
                      :title="
                        createActionMenu(selectedUser?.status, 'Account', 'Delete')?.buttonText
                      "
                      color="default"
                      rounded="sm"
                      :disabled="auth.user?.index === id"
                  ...
                  <BaseButton class="w-24" :disabled="!meta.dirty" @click="resetForm"
                    >Reset</BaseButton
                  >
                </div>
              </div>
              ...
              <form method="POST" action="" class="w-full pt-4 pb-16" @submit.prevent="onSubmit">
                <div class="flex justify-between items-center">
                  <div class="pe-8 flex-1">
                    <div class="max-w-2xl space-y-12 py-8">
                      ...
                      <BaseMessage
                        ...
                        @close="() => setErrors({})"
                      >
                        This form has {{ fieldsWithErrors }} errors, please check them before
                        submitting
                      </BaseMessage>

                      <TairoFormGroup
                        label="Personal Details"
                        sublabel="Edit or view the admin user's personal information."
                      >
                      ...
                      <TairoFormGroup
                        ...
                        sublabel="Update or review the admin user's professional background and role-related information."
                      >
                        <div class="grid grid-cols-12 gap-4">
                      ...
                      <TairoFormGroup
                        ...
                        sublabel="Manage any other relevant information related to the admin user."
                      >
                        <div class="grid grid-cols-12 gap-4">
                          <div class="col-span-12">
                            <Field
                  ...
                  <div
                    class="hidden xl:flex h-full flex-col bg-primary-600 dark:bg-muted-900 drop-shadow-md p-6 py-12 rounded-md mx-8 2xl:mx-auto w-full max-w-sm !mr-8 mt-[-280px]"
                  >
                    <!-- user avatar  -->
                    <TairoFormGroup>
                    ...
                    <div
                      class="border-muted-200 dark:border-muted-700 mt-auto border-t pt-3 space-y-2"
                    >
                      <div class="flex items-end gap-2">
                        <Icon
                      ...
                      <div class="flex items-end gap-2">
                        ...
                        <span class="text-muted-200 dark:text-muted-300 text-sm">
                          {{
                            values && values.department && values.department.length !== 0
          ...
          <div v-else-if="activeValue === 'permission'">
            <UsersAdminPermission />
          </div>
        </BaseTabSlider>
      </div>
    </BaseCard>
