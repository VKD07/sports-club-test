...
const role = ref('')
const schoolFilter = ref(props.school ? [props.school] : [])
...
const downloadFunc = async () => {
  ...
  const { data, error } = await useAdminReadService('/api/admin/all-school-account/export', {
    query,
  })
  ...
  if (responseData) {
    ...
    link.href = url
    link.setAttribute('download', `users_school_export_${Date()}.csv`)
...
watch(schoolFilter, () => {
  handleSelectedValue(2, '')
  locationRef.value?.clearSelectedValue()
})

watch([filter, role, schoolFilter, location, dateStart, dateEnd, sorting, perPage], () => {
  router.push({
    query: {
      page: undefined,
    },
...
onMounted(async () => {
  if (schoolFilter.value.length !== 0) {
    locations.value = await getSchoolLocationsByName(schoolsList.value, schoolFilter.value[0])
  }
  ...
  <TairoContentWrapper>
    <div class="flex gap-4 lg:items-center justify-between p-4 my-2 flex-col lg:flex-row">
      <div>
        ...
        <BaseText size="sm" class="text-muted-600 dark:text-muted-400">
          View and manage the complete list of all school users in the system.
        </BaseText>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <BaseButton color="primary" class="w-full sm:w-32" @click="goToAddUser('school')">
    ...
    <div>
      ...
      <div v-else class="space-y-2 pt-6">
        ...
        >
          <FlexTableRow
            v-for="(item, index) in data?.School_User?.data?.data"
            :key="index"
            shape="rounded"
            spaced
          >
            <template #start>
              <FlexTableStart
                ...
                :subtitle="item.role"
                :avatar="item.school_admin_profile?.profile_picture"
            ...
            <template #end>
              <FlexTableCell
                ...
                @sort-column="columnSort"
              >
                <span class="text-muted-500 dark:text-muted-400 font-sans text-sm break-all">
                  {{ item.school }}
                </span>
              </FlexTableCell>
              ...
              >
                <span class="text-muted-600 dark:text-muted-400 font-sans text-sm break-all">
                  {{ item.email }}
                </span>
              </FlexTableCell>
